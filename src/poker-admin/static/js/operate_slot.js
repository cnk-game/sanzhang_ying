...
function operate_slot_query(beginY, beginM, beginD, endY, endM, endD) {
    if (isWaitQuery) return;
    isWaitQuery = true;

    var date = { bYear:beginY, bMonth:beginM, bDay:beginD, eYear:endY, eMonth:endM, eDay:endD}

    $.getJSON("/operate_slot_query", date, function (data) {
        if (data == null) return;
...
function query_list(pageIdx) {
    operate_slot_query( beginYear, beginMonth, beginDay, endYear, endMonth, endDay)
}
