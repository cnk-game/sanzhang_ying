...
function initMenu() {
    ...
    $("#charm_mgr").addClass("active")

}

function initTable(table_id, tableTitle, tableHeadArr) {
    $("#" + table_id + "_h4").text(tableTitle)
    ...
    $("#" + table_id).append(dom_thead).append(dom_tbody)
}

function clear_table(table_id) {
    var rownum=$("#"+table_id+" tr").length - 2;
    for (var i = 0; i < rownum; i++) {
        $("#"+table_id+" tr:eq(2)").remove();
    }
}

function operate_charm_query(beginY, beginM, beginD, endY, endM, endD) {
    // if (isWaitQuery) return;
    isWaitQuery = true;

    var date = { bYear:beginY, bMonth:beginM, bDay:beginD, eYear:endY, eMonth:endM, eDay:endD}
    $.getJSON("/operate_charm_query", date, function (data) {
        if (data == null) return;
        clear_table("data_table_1")
        clear_table("data_table_2")
        clear_table("data_table_3")
        $("#charm_count").text(data.AllCharm);
        $("#charm_used_count").text (data.AllExpendCharm);
        console.log(data.DayExchanged)
        $.each(data.DayExchanged, function(i, item) {
            console.log(item);
            var row = "<tr>"
            row += "<td>" + item.Date + "</td>"
            row += "<td>" + item.ExpendCharm + "</td>"
            
            row += "</tr>"
            console.log(row)
            $("#data_table_1 tr:last").after(row);
        });
        $.each(data.FinePool, function(i, item) {
            var row = "<tr>"
            row += "<td>" + item.Date + "</td>"
            row += "<td>" + item.GiftGold + "</td>"
            
            row += "</tr>"
            $("#data_table_2 tr:last").after(row);
        });
        $.each(data.BadPool, function(i, item) {
            var row = "<tr>"
            row += "<td>" + item.Date + "</td>"
            row += "<td>" + item.GiftGold + "</td>"
            
            row += "</tr>"
            $("#data_table_3 tr:last").after(row);
        });
        isWaitQuery = false;
    });
}
...
function query_list(pageIdx) {
    operate_charm_query( beginYear, beginMonth, beginDay, endYear, endMonth, endDay)
}
...
$(document).ready(function () {
    ...
    initTable("data_table_1","日消耗魅力值", ["日期", "魅力值"])
    initTable("data_table_2","赞消耗金币", ["日期", "金币数"])
    initTable("data_table_3","踩消耗金币", ["日期", "金币数"])
