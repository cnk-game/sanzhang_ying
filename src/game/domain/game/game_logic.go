...
func (logic *GameLogic) CompareCardsByPos(pos1, pos2 int) bool {
	result := logic.CompareCards(logic.GetCards(pos1), logic.GetCards(pos2))
	if logic.bossPos < 0 {
		return result
	}

	// 有机器人boss
	if pos1 == logic.bossPos {
		if result {
			return result
		}
		// 需要为机器人换牌
		cardType := GetCardType(logic.GetCards(pos2))
		if cardType == CARD_TYPE_BAO_ZI {
			// 替换成更大的豹子
			logic.changeGreaterBaoZhi(logic.GetCards(pos2), pos1)
			glog.V(2).Info("===>BOSS机器人换牌为更大豹子pos1:", pos1)
		} else if cardType == CARD_TYPE_SHUN_JIN {
			// 替换成豹子
			logic.changeBaoZhi(pos1)
			glog.V(2).Info("===>BOSS机器人换牌为金花pos1:", pos1)
		} else if cardType == CARD_TYPE_JIN_HUA {
			// 替换成同花顺，不成则替换成豹子
			if !logic.changeShunJin(pos1) {
				logic.changeBaoZhi(pos1)
			}
			glog.V(2).Info("===>BOSS机器人换牌为顺金pos1:", pos1)
		} else {
			// 随机替换成金花以上
			r := rand.Int() % 100
			if r < 70 {
				// 替换成金花
				logic.changeJinHua(pos1)
				glog.V(2).Info("===>BOSS机器人换牌为金花pos1:", pos1)
			} else if r >= 70 && r < 90 {
				// 替换成顺金
				if !logic.changeShunJin(pos1) {
				...
				// 替换成豹子
				logic.changeBaoZhi(pos1)
				glog.V(2).Info("===>BOSS机器人换牌为豹子pos1:", pos1)
			}
		}
	} else if pos2 == logic.bossPos {
		if !result {
			return result
		}
		// 需要为机器人换牌
		cardType := GetCardType(logic.GetCards(pos1))
		if cardType == CARD_TYPE_BAO_ZI {
			// 替换成更大的豹子
			logic.changeGreaterBaoZhi(logic.GetCards(pos1), pos2)
			glog.V(2).Info("===>BOSS机器人换牌为更大豹子pos2:", pos2)
		} else if cardType == CARD_TYPE_SHUN_JIN {
			// 替换成豹子
			logic.changeBaoZhi(pos2)
			glog.V(2).Info("===>BOSS机器人换牌为豹子pos2:", pos2)
		} else if cardType == CARD_TYPE_JIN_HUA {
			// 替换成同花顺，不成则替换成豹子
			if !logic.changeShunJin(pos2) {
				logic.changeBaoZhi(pos2)
			}
			glog.V(2).Info("===>BOSS机器人换牌为顺金pos2:", pos2)
		} else {
			// 随机替换成金花以上
			r := rand.Int() % 100
			if r < 70 {
				// 替换成金花
				logic.changeJinHua(pos2)
				glog.V(2).Info("===>BOSS机器人换牌为金花pos2:", pos2)
			} else if r >= 70 && r < 90 {
				// 替换成顺金
				if !logic.changeShunJin(pos2) {
					logic.changeBaoZhi(pos2)
